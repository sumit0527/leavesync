...
export default function Profile() {
  ...
  const exportToExcel = () => {
    ...
    XLSX.utils.book_append_sheet(wb, statsWs, 'Leave Stats');

    // Leave Allocations sheet
    const allocData = [
      ['Leave Type', 'Total Allocated', 'Used', 'Remaining'],
      ...allocations.map((a) => [
        a.leave_type?.name || 'N/A',
        a.total_allocated,
        a.used,
        a.remaining,
      ]),
    ];
    const allocWs = XLSX.utils.aoa_to_sheet(allocData);
    XLSX.utils.book_append_sheet(wb, allocWs, 'Allocations');

    XLSX.writeFile(wb, `profile_report_${format(new Date(), 'yyyy-MM-dd')}.xlsx`);
  };
          ...
          <CardContent className="flex flex-col gap-3 md:flex-row">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={downloadFullReport}>
