...
export interface AuthContextType {
  ...
  signUpWithUsername: (username: string, password: string, fullName: string, phone: string, email: string, address: string, departmentId: string, adminSecret?: string, requestedRole?: 'staff' | 'admin' | 'principal' | 'main_admin' | 'director', options?: { collegeUnit?: 'junior' | 'senior' | 'pharmacy'; adminDesignation?: 'principal' | 'uh' }) => Promise<{ error: Error | null }>;
